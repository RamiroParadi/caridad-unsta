...
export default function MaterialesDonationsPage() {
  ...
  useEffect(() => {
    ...
    // Filtrar solo donaciones de materiales de estudio
    filtered = filtered.filter(donation => 
      donation.section.name.toLowerCase().includes('materiales') ||
      donation.section.name.toLowerCase().includes('estudio') ||
      donation.section.name.toLowerCase().includes('libros') ||
      donation.section.name.toLowerCase().includes('útiles')
    )
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({
          ...
          description: "No se pudieron cargar las donaciones de materiales",
          variant: "destructive",
        })
      }
    } catch (error) {
  ...
  const exportDonations = () => {
    const csvContent = [
      ['Donante', 'Email', 'Descripción', 'Estado', 'Fecha', 'Sección'],
    ...
    link.setAttribute('download', `donaciones_materiales_${new Date().toISOString().split('T')[0]}.csv`)
  ...
  if (isLoading) {
    return (
      <div className="space-y-6">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">Donaciones de Materiales de Estudio</h2>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">Donaciones de Materiales de Estudio</h2>
          <p className="text-muted-foreground">
            Gestiona libros, útiles escolares y material educativo
          </p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {filteredDonations.filter(d => d.status === 'PENDIENTE').length}
            </div>
            <p className="text-xs text-muted-foreground">
              Por confirmar
            </p>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {filteredDonations.filter(d => d.status === 'CONFIRMADA').length}
            </div>
            <p className="text-xs text-muted-foreground">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {filteredDonations.filter(d => d.status === 'RECHAZADA').length}
            </div>
            <p className="text-xs text-muted-foreground">
              ...
              <div className="text-center py-8">
                ...
                <p className="text-muted-foreground">
                  {searchTerm || statusFilter !== 'ALL' || sectionFilter !== 'ALL'
                    ...
                    : 'Aún no hay donaciones de materiales de estudio registradas.'}
                </p>
              </div>
            )}
          </div>
